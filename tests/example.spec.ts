...
test("it sends the valid form data to a result page that displays inputs", async ({
  page,
}) => {
  await page.goto(url);
...
test("validations for controlled component first name", async ({ page }) => {
  ...
  await expect(
    page.getByText("name should start with custom.name")
  ).toBeVisible();
...
test("display derived value from first name and last name ", async ({
  page,
}) => {
  await page.goto(url);
